...
// var results = stmt.executeQuery('select distinct  `first_name` "Forenames",`last_name` "Surname", `admin-dob` "Dob",  `shipping_postcode` "Postcode", id "Clan Membership Number" from wp_member_db db JOIN wp_order_product_customer_lookup pd on pd.user_id = db.id where product_id=2118 AND db.cc_member="yes" AND status in ("wc-processing", "wc-completed") AND YEAR(pd.order_created)>YEAR(CURDATE())-1 order by `first_name` ASC;');
